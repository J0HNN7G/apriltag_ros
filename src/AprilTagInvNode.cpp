...
class AprilTagInvNode : public rclcpp::Node {
public:
    AprilTagInvNode(const rclcpp::NodeOptions& options);

    ~AprilTagInvNode() override;
...
RCLCPP_COMPONENTS_REGISTER_NODE(AprilTagInvNode)


AprilTagInvNode::AprilTagInvNode(const rclcpp::NodeOptions& options)
  : Node("apriltag", options),
    // parameter
    cb_parameter(add_on_set_parameters_callback(std::bind(&AprilTagInvNode::onParameter, this, std::placeholders::_1))),
    td(apriltag_detector_create()),
    // topics
    sub_cam(image_transport::create_camera_subscription(
        this,
        this->get_node_topics_interface()->resolve_topic_name("image_rect"),
        std::bind(&AprilTagInvNode::onCamera, this, std::placeholders::_1, std::placeholders::_2),
...
AprilTagInvNode::~AprilTagInvNode()
{
    apriltag_detector_destroy(td);
    tf_destructor(tf);
}

void AprilTagInvNode::onCamera(const sensor_msgs::msg::Image::ConstSharedPtr& msg_img,
                            const sensor_msgs::msg::CameraInfo::ConstSharedPtr& msg_ci)
{
    ...
    for(int i = 0; i < zarray_size(detections); i++) {
        ...
        // set header frame name by generic tag name or configured tag name
        ...
        if(estimate_pose != nullptr) {
            ...
            inv_transform.setOrigin(tf2::Vector3(tf.transform.translation.x, tf.transform.translation.y, tf.transform.translation.z));
            inv_transform.setRotation(tf2::Quaternion(tf.transform.rotation.x, tf.transform.rotation.y, tf.transform.rotation.z, tf.transform.rotation.w));
            inv_transform = inv_transform.inverse();

            tf.transform.translation.x = inv_transform.getOrigin().x();
            tf.transform.translation.y = inv_transform.getOrigin().y();
            tf.transform.translation.z = inv_transform.getOrigin().z();
            tf.transform.rotation.x = inv_transform.getRotation().x();
            tf.transform.rotation.y = inv_transform.getRotation().y();
            tf.transform.rotation.z = inv_transform.getRotation().z();
            tf.transform.rotation.w = inv_transform.getRotation().w();
        }
...
AprilTagInvNode::onParameter(const std::vector<rclcpp::Parameter>& parameters)
{
